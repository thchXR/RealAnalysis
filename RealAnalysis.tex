...
\ITEMS
	...
	\item {\bf 闭包} 集合 $\overline A\subseteq \R^n$  称为 $A$ 的闭包，如果 $\overline A$ 是包含 $A$ 的最小闭集，等价于
	\[
		\overline A = A \cup A'
	\]
	
\EXP 在 $\R$ 中，$(0, 1]$ 的闭包 是 $[0, 1]$ 。
...
\no {\bf 注2} 由上述定义易知：闭集就是对极限运算封闭的点集，也就是说：对该集合内任一柯西序列，其极限点必在该集合内。\\	
...
\ITEMS 
	...
	\item $\mathcal{B}(\R^n)$ 中含 $\R^n$ 中所有 \(F_\sigma\) 集，其中我们把可数无穷多个开集的交集称为 \(G_\delta\) 集 
...
从闭区间 $I = [0,1]$ 开始，我们把 $I$ 三等分，并去掉中间的开区间 $\left(\frac13,\frac23\right)$，余下如图两个闭区间 $\left[0,\frac13\right]$ 和 $\left[\frac23,1\right]$，分别记为 $I_{11}$ 和 $I_{12}$。然后，再把这两个区间的中间三分之一都去掉，留下四个区间 $I_{21},I_{22},I_{23},I_{24}$ ，接下来不断重复以上过程：
...
\begin{tikzpicture}
  ...
  % 第1层
  \draw[thick] (0,-1) -- (3,-1)
  	node[left=45pt,above=2pt] {$I_{11}$};
  \draw[thick] (6,-1) -- (9,-1) 
    node[left=45pt,above=2pt] {$I_{12}$};
  ...
  % 第2层
  \draw[thick] (0,-2)   -- (1,-2)
  	node[left=15pt,above=2pt] {$I_{21}$};
  \draw[thick] (2,-2) -- (3,-2)
  	node[left=15pt,above=2pt] {$I_{22}$};
  \draw[thick] (6,-2) -- (7,-2)
  	node[left=15pt,above=2pt] {$I_{23}$};
  \draw[thick] (8,-2) -- (9,-2) 
    node[left=15pt,above=2pt] {$I_{24}$};
...
\[
	\mathcal{C} = \bigcap_{i=1}^\infty\left(\bigcup_{j=1}^{2^i}I_{ij}\right)
\]
不难看出，$\mathcal{C}$ 是非空闭集，因为上述每个闭区间 $I$ 的端点必然不会被去掉，从而必属于 $\mathcal{C}$。\\
 
\subsubsection{康托三分集的基数}

不难看出，康托三分集 $\mathcal{C}$ 中每一个数与三进制小数 $0.a_1a_2a_3\cdots a_n\cdots \mbox{每个 $a_n$ 是 $0$ 或 $2$}$ 一一对应，所以
\[
