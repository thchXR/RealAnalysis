...
我们定义经过以上过程余下的所有点组成的集合为康托尔三分集（Cantor），记作 $\mathcal{C} \subseteq R$
\[
	\mathcal{C} = \bigcap_{i=1}^\infty\left(\bigcup_{j=1}^{2^i}I_{ij}\right)
\]
显然以上过程中取得的每个端点都属于 $\mathcal{C}$ ，且 $\mathcal{C}$ 是非空闭集

\subsubsection{康托尔集的基数}

我们参考求实数集的基数时所用的方法，在康托尔集 $\mathcal{C}$ 与一个三进制小数 $0.a_1a_2a_3\cdots a_n\cdots$ 建立一一对应关系，该三进制小数中有无穷多个 $a_n$ 是2，可知
\[
