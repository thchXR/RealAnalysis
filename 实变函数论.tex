...
\section{$\R^n$上的拓扑（Topology）}
% $n$维的欧氏空间上的拓扑（Topology 地形）
...
我们把在$\R^n$空间上的开球定义为 % 这里定义开球所用的距离（模）是否需要提前定义
\[
...
\pro 
	...
	% 用定义1推导定义2
	% 根据开球的定义可知开球$B(x;r)$中一点总能构造一个更小的开球属于$B(x;r)$
	% 所以开球$B(x;r)$等于$B(x;r)$的内部
	% => 那么开球的并集也应该等于其内部
	% => 所以开集等于其内部
	% => 定义1
...
\subsubsection{可分}
拓扑空间 $X$ 称为可分的（separable），当且仅当它包含一个可数的稠密子集
\smallskip

\EXP $\R^n$ 是可分的，因为 $\mathbb{Q}^n$ 的基数为 $\aleph_0$（可数集）

\subsubsection{ $\R^n$ 上的连续性}

对于在 $\R^n$ 上的映射 $f:X\mapsto\R$，其中 $X\subseteq\R^n$，映射 $f$ 在 $X$ 上连续
